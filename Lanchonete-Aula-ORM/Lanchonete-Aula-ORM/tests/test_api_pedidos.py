...
async def test_pedido_limite_atingido(client):
    await client.post("/clientes", json={"cpf": "99988877766", "nome": "Cliente Limite"})
    await client.post("/produtos", json={"codigo": 10, "valor": 15, "tipo": 1})
    await client.post("/produtos", json={"codigo": 20, "valor": 25, "tipo": 1})


    resposta = await client.post("/lanchonete/pedidos", json={
        "cpf": "99988877766", 
        "cod_produto": 10, 
        "qtd_max_produtos": 1
    })
    assert resposta.status_code == 200
    cod_pedido = resposta.json()["codigo"]

    resposta_falha = await client.put(f"/lanchonete/pedidos/{cod_pedido}/itens", json={"cod_produto": 20})
